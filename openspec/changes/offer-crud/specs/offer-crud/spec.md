...
- **WHEN** user creates an offer with title "Développeur PHP Senior"
- **THEN** system generates slug "developpeur-php-senior"
...
The system SHALL use OfferRequest form request class for store and update validation. Validation rules SHALL enforce: title required|string|max:255, description required|string, required_skills required|array|min:1, soft_skills array, min_experience_level nullable|in:junior,confirmed,senior,expert, employment_type nullable|in:full_time,part_time,contract,internship,temporary.
